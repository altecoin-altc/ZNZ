...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in zenzo.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in zenzo.qrc)
...
/** Class encapsulating ZENZO Core startup and shutdown.
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // ZENZO: URIs or payment requests:
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(zenzo_locale);
    Q_INIT_RESOURCE(zenzo);
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("ZENZO Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // zenzo: links repeatedly have their payment requests routed to this process:
