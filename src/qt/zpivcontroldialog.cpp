    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zZNZ Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zZNZ.";
            else if (!mint.isSeedCorrect)
                strReason = "The zZNZ seed used to mint this zZNZ is not the same as currently hold in the wallet";
            else
